...
public class BorderOnlyMapGenerator<T> : IMapGenerator<T> where T : IMap, new()
{
    ...
    public T CreateMap()
    {
        ...
        foreach ((Vector2I tilePosition, Tile tile) tileData in map.GetTilesInRows(0, _height - 1))
        {
            map.SetTile(tileData.tilePosition.X, tileData.tilePosition.Y, new Tile(Tile.Type.Block));
        }

        foreach ((Vector2I tilePosition, Tile tile) tileData in map.GetTilesInColumns(0, _width - 1))
        {
            map.SetTile(tileData.tilePosition.X, tileData.tilePosition.Y, new Tile(Tile.Type.Block));
        }
