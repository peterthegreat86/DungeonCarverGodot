namespace DungeonCarver
{
    ...
    public class Map : IMap
    {
        ...
        public IEnumerable<(Vector2I tilePosition, Tile tile)> GetAllTiles()
        {
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    yield return (new Vector2I(x, y), GetTile(x, y));
                }
            }
        }

        public IEnumerable<(Vector2I tilePosition, Tile tile)> GetTilesInRows(params int[] rowNumbers)
        {
        ...
        public IEnumerable<(Vector2I tilePosition, Tile tile)> GetTilesInColumns(params int[] columnNumbers)
        {
        ...
        public IEnumerable<(Vector2I tilePosition, Tile tile)> GetTilesInSquare(int xCenter, int yCenter, int distance)
        {
        ...
        public IEnumerable<(Vector2I tilePosition, Tile tile)> GetCellsAlongLine(int xOrigin, int yOrigin, int xDestination, int yDestination)
        {
            ...
            while (true)
            {
                yield return (new Vector2I(xOrigin, yOrigin), GetTile(xOrigin, yOrigin));
        ...
        public static T Create<T>(IMapGenerator<T> mapCreationStrategy) where T : IMap
        {
            ...
            {
                GD.PushError(nameof(mapCreationStrategy) + "Map creation strategy cannot be null");
            }
        ...
        public virtual T Clone<T>() where T : IMap, new()
        {
            ...
            foreach ((Vector2I tilePosition, Tile tile) tileData in GetAllTiles())
            {
        ...
        public virtual bool IsBorderTile(Vector2I position)
        {
